...
from class_and_func.estimator_class import univariate_spectral_noised_estimator, old_univariate_spectral_noised_estimator
...
if __name__ == "__main__":
    ...
    mu = np.array([[1.0]])# 1.0 or 1/8
    alpha = np.array([[0.5]])# 0.5 or 1/8
    beta = np.array([[1.0]])
    ...
    horizons = [4000]#[250, 500, 1000, 2000, 4000, 8000]
    max_time = horizons[-1]
        ...
        for K_idx, K_func in enumerate(K_func_list):
            ...
            for horizon in horizons:
                ...
                for j, fixed_parameter in enumerate(fixed_parameter_list):
                    ...
                    np.savetxt("saved_estimations_univariate/horizons_original/" + str(horizon) + "univariate_horizon_" + str(K_func_name[K_idx]) + "_"+ fixed_list[j] + "_" + str(np.round(noise, 2)) + ".csv", estimations,
                               delimiter=",")
                end_time = time.time()
