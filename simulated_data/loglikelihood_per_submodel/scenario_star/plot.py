...
if __name__ == "__main__":
    ...
    theta = np.concatenate((mu.ravel(), alpha.ravel(), beta.ravel(), noise))
    ...
    noisedNlogN = pd.read_csv('saved_estimations/5partition_2neurons_estimation_NlogN.csv', sep=',', header=None, index_col=None).to_numpy()
    for i in range(4):
        ax[i].scatter(positions[indices[i]:indices[i+1]] + 0.2, theta[indices[i]:indices[i+1]], c="k", zorder=10, label="True parameters")
        ...
        ax[i].scatter(positions[indices[i]:indices[i+1]], theta[indices[i]:indices[i+1]], c="k", zorder=10)

        for patch in bplot['boxes']:
            patch.set_facecolor("paleturquoise")
    alpha_est_noised = noisedNlogN[:, dim:dim+dim**2].reshape((noisedNlogN.shape[0],dim, dim))
    mean = np.mean(noisedNlogN[:,:], axis=0)
    print("Estimated parameters: ", np.round(mean,2))#, np.round(np.std(noisedNlogN, axis=0, ddof=1), 2))
    print("noisedNlogN percentages: ", np.mean(noisedNlogN[:,:] < 2e-16, axis=0))

    noisedNlogN = pd.read_csv('saved_estimations/5partition_2neurons_estimation_NlogN_id3.csv', sep=',', header=None,
    ...
    plt.savefig("real_simulated_partition_estimation_boxplots.pdf", format="pdf", bbox_inches="tight")
